...
"""Core API configuration and entrypoint."""
...
init_logging(logging_env_var_start="THOTH_MOLDAVITE_LOG_")

_THOTH_API_HTTPS = bool(int(os.getenv("THOTH_API_HTTPS", 1)))
_LOGGER = logging.getLogger("moldavite")
_LOGGER.setLevel(
    logging.DEBUG if bool(int(os.getenv("THOTH_MOLDAVITE_DEBUG", 0))) else logging.INFO
)
...
metrics.info("moldavite_api_info", "Moldavite API info", version=__service_version__)
